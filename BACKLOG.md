...
- **Interactive mode** — A guided "hand-holding" mode that prompts the user for URL, output directory, overwrite preference, etc. step by step instead of requiring CLI arguments. Could be the default when `ohmd` is run with no arguments (instead of printing help). Pairs well with persistent config ("save these as defaults?").
- **Persistent config file** — User config at `~/.config/ohmd/config.toml` (or platform-appropriate location) for default output directory, API key, force behavior, and other settings. CLI args override config, config overrides built-in defaults.
